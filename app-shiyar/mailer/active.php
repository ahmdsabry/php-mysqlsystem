...
$checkCode = $database->prepare("SELECT SECURITY_CODE FROM userss WHERE SECURITY_CODE = :SECURITY_CODE");
$checkCode->bindParam("SECURITY_CODE",$_GET['code']);
$checkCode->execute();
if($checkCode->rowCount()>0){
   
$update = $database->prepare("UPDATE userss SET SECURITY_CODE = :NEWSECURITY_CODE ,
 active=true WHERE SECURITY_CODE = :SECURITY_CODE");
